...
//  VPN Guard
//
//  Created by Ali Fakih on 5/8/20.
...
public class SwiftyPing: NSObject {
    ...
    init(destination: Destination, configuration: PingConfiguration, queue: DispatchQueue) {
        ...
        // ...and a socket...
        self.socket = CFSocketCreate(kCFAllocatorDefault, AF_INET, SOCK_DGRAM, IPPROTO_ICMP, CFSocketCallBackType.dataCallBack.rawValue, { socket, type, _, data, info in
    ...
    private func sendPing() {
        ...
        currentQueue.async {
            ...
            guard let icmpPackage = self.createICMPPackage(identifier: UInt16(self.identifier),
                                                           sequenceNumber: UInt16(self.sequenceIndex),
                                                           payloadSize: Int(self.configuration.payloadSize)), let socket = self.socket else { return }
            ...
            if socketError != .success {
                ...
                let response = PingResponse(identifier: self.identifier,
                                            ...
                                            duration: Date().timeIntervalSince(self.sequenceStart ?? Date()),
                                            error: error)
